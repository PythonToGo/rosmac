...
| D14 *(제안 — P4.4 착수 전 승인 필요)* | VM 워크스페이스 전달 = **push 복사 방식** (`rosmac push`), 마운트 아님 | 마운트는 VM 재생성 + virtiofs/9p 성능·안정성 미검증 + 산출물 역오염 위험. 복사는 명시적·안전 | v0.2에서 마운트 재평가 (대형 ws 복사 시간이 실측으로 문제될 때) |
