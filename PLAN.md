...
> 최종 수정: 2026-07-09
> 상태: **Phase 4 완료 (E2E ALL PASS 38s, 2026-07-08) → Phase 5 진행 중.
> KI-28은 4차 조사(자연 재발, 2026-07-09)로 **원인 확정** — 규칙 없는 lima VM의
> UDP 특정주소 하이잭. NECP 반증, 무재부팅 처방 확립. 제품 대응 완료 — doctor C17 (E.16, 2026-07-09).
...
(b) 출판 (JOSS 소프트웨어 페이퍼 + ROSCon 발표 제안 — 준비까지만, 제출은 보류 D13),
(c) 포트폴리오 피쳐. Phase 4(기능 보강) → 5(제품화) → 6(런칭) → 7(출판)이 이
트랙이다. 공개의 전제 두 가지: ① 외부 워크스페이스 수용 능력(Phase 4 E2E 게이트 —
2026-07-07 실사용에서 실증된 갭 해소), ② "모르는 사람의 맥에서 문서만으로
재현"(Phase 5.6 게이트). 이 기준을 통과하지 못한 상태로 공개하지 않는다.
...
| 4 | 기능 보강 (실사용 갭) | `rosmac deps/ps/push` + colcon 기본값 주입 | "외부 ROS2 워크스페이스" 시나리오 E2E — deps 해결 → 무플래그 빌드 → ps 관찰 → linux 전용 패키지 VM 빌드 | [phase4](docs/plan/phase4-features.md) |
| 4.5 | 완성도 보강 (**비게이트** — 병행/생략 가능) | 사용자 프리셋, `rosmac env`, RViz2 문서·doctor, Foxglove 레이아웃 재조사 | 착수한 태스크만 AC 완주 — Phase 5 진행을 막지 않음 | [phase4.5](docs/plan/phase4.5-polish.md) |
...
제품화 트랙은 4 → 5 → 6 → 7 순서 강제 (Phase 4 E2E 없이 제품화 금지,
5.6 재현성 게이트 없이 공개 금지, 공개 repo 없이 JOSS 제출 준비 불가).
Phase 3(GPU)과 Phase 4.5(완성도)는 비게이트 — 언제든 병행/생략 가능.
...
| D14 | VM 워크스페이스 전달 = **push 복사 방식** (`rosmac push`), 마운트 아님 (사용자 승인 2026-07-08) | 마운트는 VM 재생성 + virtiofs/9p 성능·안정성 미검증 + 산출물 역오염 위험. 복사는 명시적·안전 | v0.2에서 마운트 재평가 (대형 ws 복사 시간이 실측으로 문제될 때) |
| D15 | 실로봇 연결 = **로봇 쪽 zenoh-bridge-ros2dds + 맥 브리지 `-e tcp/<robot>:7447` 추가** (스타형, 맥이 접속 주체). 비목표: 로봇에 무설치(복붙 가이드만 제공), v1은 동일 배포판(Humble)만, 신뢰 LAN 평문 TCP (사용자 R0 실행 지시 2026-07-09, [E.15 계획](docs/plan/e15-real-robot.md)) | 단일 TCP 경계 모델의 대칭 확장 — DDS 멀티캐스트가 WiFi를 안 건너고, 로봇 방화벽은 인바운드 7447 하나. 신규 커맨드 없이 up/status/ps/doctor 확장 | R1 스파이크에서 VM↔로봇 트랜지티브 라우팅 불가 시 토폴로지 보강(VM 브리지에도 엔드포인트). TLS·다중 로봇·혼합 배포판은 수요 발생 시 |
| D16 | VM→맥 파일 회수(bag 등) = **전용 `rosmac pull` 도구 없이 `limactl cp` 안내로 충분** (E.14 실측 2026-07-09) | `limactl cp -r rosmac:/path ~/dest` 실측 동작 — bag 디렉토리 회수 확인. 커맨드 표면 최소 유지(D15와 같은 원칙), lima 내장 기능 재발명 회피 | 사용 빈도가 높아져 UX 불만이 실측되면 v0.2에서 `rosmac pull` 재검토 |
...
| R7 | 업스트림 드리프트 — RoboStack 채널/zenoh 릴리스/lima 변경으로 **신규 설치**가 깨짐 (기존 사용자는 핀으로 보호되나 신규 유입이 죽음) | 고 (공개 후 신뢰 직결) | 중 | weekly CI가 실제 env 생성·URL 검증 (P5.4), 실패 시 자동 이슈. 버전 핀 + doctor 매트릭스 | Phase 5부터 상시 |
