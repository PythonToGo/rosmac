...
> 신청 등 실행은 사용자가 별도 지시할 때까지 하지 않는다 (7.4는 보류 상태).
> 착수 조건: Phase 6 완료 (공개 repo가 출판의 전제 — JOSS는 공개 저장소 요구)
...
| venue | 형태 | 적합성 | 비고 |
|---|---|---|---|
| **JOSS** (Journal of Open Source Software) | 소프트웨어 페이퍼 (paper.md 0.5~2p) + 코드 리뷰 | **1순위** — "연구를 가능하게 하는 도구" 카테고리에 정확히 부합, 리뷰가 곧 품질 검증, DOI 발급 | 무료, 공개 리뷰. Phase 5~6 산출물이 요구사항과 거의 1:1 |
...
```
7.1 평가 데이터 확정 ─→ 7.3 paper.md 작성 ─→ 7.4 제출 (사용자)
7.2 JOSS 요건 갭 분석 ─┘        ROSCon 초록은 7.3에서 파생
```

---

## 7.1 평가 데이터 확정 (재현 가능한 벤치마크)
...
3. 결과를 `docs/plan/phase7-results.md` + 페이퍼 표로 정리 (HW/버전 명기).
...
|---|---|---|
| OSI 라이선스 | LICENSE (6.3) | |
| 설치 문서·의존성 명시 | README Quickstart (5.6 검증) | |
| 자동 테스트 | unit + CI (5.4) | |
| 예제/사용 문서 | workflow.en.md, 프리셋 (6.1) | |
| 기여 가이드 | CONTRIBUTING (6.3) | |
| Statement of need | paper.md (7.3) | |
...
- [ ] 모든 정량 주장에 7.1 데이터 또는 phase-results 근거
- [ ] 사용자 리뷰 1회 반영

---

## 7.4 제출 (보류 — 사용자 지시 시에만 활성화, D13)
