...
> **⚠️ S0 정정 (2026-07-12) — 아래 N2/N3의 "스코핑 필수/구현" 결론은 폐기됨.**
> E.20 S0 게이트 실측: 무스코프 브리지가 nav2 풀스택을 정상 라우팅하고 맥 goal
> 3/3 SUCCEEDED. N2 실패의 진짜 원인은 엔티티 포화가 아니라 **스택 재기동 시
> 브리지 라우트 잔재(KI-17 계열)** — 동일 스택이 브리지 재시작 전 0/6 → 후 4/4.
> 스코핑이 통했던 건 스코프 적용이 VM 브리지를 재시작한 부수효과였음. **진짜
> 해법: `rosmac sim`이 시작 시 브리지 세션 리셋**(sim.py `_reset_bridge_session`).
> 스코핑 메커니즘·bridge_allow는 제거됨. KI-30 재작성 참조. 아래 원 기록은 사료로 보존.

#### N2 결과 — ⚠️ 조건부 PASS (2026-07-11 실측) — ~~브리지 스코핑 필수 발견 (KI-30)~~ [S0에서 폐기]
