...
## E.7 [배포 전 필수] rosmac 업그레이드 경로 — 핀 마이그레이션 — ✅ 완료 (2026-07-10)
...
- **배경 (실측)**: config.load()가 첫 실행 때 브리지 버전/sha 전체를
  ...
  break" 선언과 정면 충돌.
- **구현** (① "저장 시 핀 제외" 안 채택 + 히스토리 마이그레이션):
  - ① `_PIN_HISTORY`(config.py): 핀 5종(bridge version/sha×2, ros.rmw,
    conda_channel)은 **save 시 파일에 쓰지 않고**, load 시 "역대 기본값"과
    일치하는 값은 버려서 코드 기본값을 따른다 → 과거에 동결된 구 config도
    파일 재작성 없이 마이그레이션(주석 보존). 역대 기본값이 아닌 값 = 사용자
    커스텀 핀으로 간주해 보존. **릴리스 절차: 핀을 올릴 때 옛 값을 히스토리에
    남길 것** (config.py 주석).
  - ② `ensure_binary`: 존재 검사 → **버전 비교**(`--version` 실측 출력
    `zenoh-bridge-ros2dds v1.9.0` 파싱, 스탬프 파일 불필요). 불일치·판독 불가 시
    재다운로드. `rosmac up`도 진입 시 보장 — 다운로드 실패(오프라인·404)면
    경고 후 설치본으로 계속, 교체 시 구버전 프로세스가 돌고 있으면 재시작 안내.
  - ③ `urlretrieve` → `urlopen(timeout=60)` 스트리밍 (B6 해소).
- **AC**: [x] 실측 — 핀 교체 전 사이클: 커스텀 핀 1.8.0 → `up`이
  "binary installed/updated (v1.8.0)" + 구버전 프로세스 경고 → 핀 제거(신형
  config) → `down --keep-vm && up` → v1.9.0 자동 복귀, C8 PASS. 다운로드 실패
  경로도 실측(존재하지 않는 1.8.6 → 404 경고 후 기존 바이너리로 up 성공).
  [x] 핀 갱신 릴리스 시나리오 유닛(히스토리에 옛 값 추가 → 옛 동결 config가
  신 핀 추종) + 커스텀 핀 보존/신형 파일 무핀 유닛 — 93 tests
  [x] 타임아웃 유닛(urlopen timeout=60 캡처)
- **잔여 (주의)**: **VM 쪽 브리지는 프로비저닝 시점 핀**으로 설치되므로 맥과
  별개 — 핀이 올라간 릴리스에서 기존 VM은 구버전 브리지로 남는다(1.8↔1.9
  상호운용은 E.15 R1 실측으로 동작 확인). 완전한 VM 업그레이드는 E.9
  (`init --recreate-vm`) 몫으로 위임.
- **시점**: **P5.5 착수 전** (게이트급) — 충족
...
## E.16 doctor — 외부 lima VM UDP 하이재커 감지 (KI-28 4차 파생) — ✅ 완료 (2026-07-09)
...
- **AC**: [x] 미수리 VM 기동 상태에서 C17 FAIL + 처방 실측 — 스파이크 VM의
  ignore 규칙을 임시 제거·재기동하니 limactl이 127.0.0.1:7410-7411 특정 바인드,
  C17 FAIL `limactl[88863]:7410, ...` + lima.yaml 처방 출력 (측정 후 즉시 복구)
  [x] `ensure_udp_ignore_rules`(2건 삽입) + VM 재시작 → C17 PASS, C8 PASS(잔여
  오염 없음 — 하이잭 창이 짧으면 좀비 정리 불필요함도 확인)
  [x] KI-28 처방 절 정합 — known-issues에 "제품 대응 완료: doctor C17" 반영,
  C8 처방 문구에 잔여 정리(③) 추가, PLAN.md 헤더 갱신. 판정은 순수 함수
