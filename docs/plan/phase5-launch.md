# Phase 5 — 오픈소스 런칭 (문서·커뮤니티·공개)

> 목표: rosmac을 공개 저장소로 전환하고, 외부인이 신뢰하고 채택할 수 있는
> 문서·커뮤니티 기반을 갖춘 뒤 v0.1.0을 런칭한다.
> 착수 조건: Phase 4 완료 (4.6 게이트 통과 — 재현성이 증명 안 된 도구를 공개하지 않는다)
...
```
5.1 영어 문서 전환 ─┐
5.2 데모 자산       ─┼→ 5.5 v0.1.0 런칭 (게이트: 사용자)
5.3 커뮤니티 인프라 ─┤
5.4 포지셔닝 문서   ─┘
```
...
1. `README.md` → 영어 재작성 (단순 번역이 아니라 공개용 재구성: 문제 → 데모 GIF →
   ...
   한국어판은 `README.ko.md`로 보존, 상호 링크.
2. `docs/workflow.md` → `docs/workflow.en.md` 영어판 (개발 루프, 함정표 포함).
...
- [ ] LICENSE·고지 검토 결과가 phase5-results.md에 기록

---

## 5.4 포지셔닝 문서 ("왜 rosmac인가")
