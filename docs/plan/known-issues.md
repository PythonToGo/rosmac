...
## KI-28. [미해결] 맥 로컬 DDS 디스커버리가 참가자 수에 따라 점진 붕괴 (lo0 멀티캐스트)
...
- **유력 가설**: macOS(Darwin 25.x) 커널의 **SO_REUSEPORT 그룹(*:7400 다중 바인드)에
  대한 멀티캐스트 배달이 부분적** — 바인드 순서/그룹 구성에 따라 일부 소켓이 배달에서
  누락. 참가자(=7400 바인더)가 늘수록 악화되는 관측과 정합
- **다음 조사 (권장 순서)**:
  ① **맥 재부팅** 후 최소 구성(브리지+talker+echo)으로 기준선 재측정 — 커널 mc 상태
    리셋으로 "어제 동작"이 재현되는지. 재현되면 "재부팅 후 N일/참가자 M개에서 재발"을
    정량화
  ② 재부팅 직후 깨끗한 환경에서 유니캐스트 전환(⑤) 재검증 — 이번 실측은 오염된
    호스트 상태에서였음
  ③ cyclonedds 소스에서 macOS SO_REUSEPORT/멀티캐스트 처리 확인 + 업스트림 이슈 검색
    ("macos multicast loopback SO_REUSEPORT")
  ④ 근본 우회 후보: 맥 쪽 DDS를 **단일 참가자로 수렴**(모든 CLI를 데몬 경유로?) 불가 →
    현실적 우회는 브리지·노드의 도메인 분리 불가… 설계 차원 검토 필요
- **임시 처방 (사용자용)**: 증상 발생 시 ① `rosmac ps`로 상태 확인 ② 맥 재부팅
  ③ `rosmac up` — 그래도 안 되면 이 KI에 재현 기록 추가
