...
## KI-28. [미해결] 맥 로컬 DDS 디스커버리 간헐 붕괴 (lo0 멀티캐스트, NECP/LNP 의심)
...
> (누적 64개·churn 600회 전부 정상). 재부팅으로 리셋되는 소켓별 수신 차단이며
> 최유력은 macOS Local Network Privacy(NECP). 강제 재현 실험은 소진, 재발 대기 상태.
...
- **임시 처방 (사용자용)**: 증상 발생 시 ① `rosmac ps`로 상태 확인 ② 위 플레이북 수집
  ③ 맥 재부팅 → `rosmac up`. 반복 발병 시 맥 쪽 유니캐스트 전환(⑩)을 제품 기본값으로
  검토(D 결정 필요 — LNP는 유니캐스트 lo0에도 개입 가능하므로 만능 아님에 유의)
