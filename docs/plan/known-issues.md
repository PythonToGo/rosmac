...
## KI-30. 브리지 라우트 잔재 — 스택 재기동 시 새 스택의 액션이 맥에서 안 보임
- **증상** (2026-07-11 실측, E.17 N2/S0): VM 시뮬 스택을 재기동한 뒤 맥에서
  `/navigate_to_pose` 액션 goal 전송이 `wait_for_server`에서 `SERVER_NOT_AVAILABLE`.
  스택은 건강(bt_navigator active, VM-로컬 goal SUCCEEDED)한데도 맥에서 액션 하위
  서비스 디스커버리가 안 됨. 토픽(/scan·/odom·/map)은 정상.
- **원인** (S0 게이트에서 규명 — 최초 "서비스 포화" 진단은 **오류**): 브리지를
  켜둔 채 스택을 재기동하면 VM 브리지가 죽은 스택들의 라우트를 누적한다(KI-17
  계열 — 비정상 종료 잔재의 sim 판). 새 스택의 액션 5개 하위 서비스 매칭이
  오염돼 맥 `wait_for_server`가 실패한다. **엔티티 수 포화가 아님** — 무스코프
  브리지는 nav2 풀스택(서비스 155·액션 12)을 정상 라우팅한다(신선한 브리지에서
  맥 goal 3/3 SUCCEEDED). 판별 실측: **동일 스택**이 브리지 재시작 전 wait_for_server
  0/6 → 재시작 후 4/4, goal 3/3.
- **해결**: `rosmac sim`이 스택 시작 시 브리지 세션을 리셋(`systemctl restart
  zenoh-bridge` + 맥 브리지 재기동) — `rosmac up`의 KI-17 방지와 동일 패턴
  (sim.py `_reset_bridge_session`). 스코핑 불필요.
- **폐기된 접근**: 프리셋별 `bridge_allow` 화이트리스트 스코핑(E.17 N2/N3에서
  일시 채택). 통했던 건 스코프 적용이 VM 브리지를 재시작한 **부수효과**였고,
  근본 해법인 브리지 리셋으로 대체하며 제거(sim.py, 커밋 참조).
- **관련**: [KI-17](#) (브리지 비정상 종료 → 라우트 잔재 2배 수신), KI-18
  (pkill 자기 매칭 — S0 디버깅 중 반복 발동).
